...
package com.mostafa.alpakka.simplealpakka.kafkatoelastic;
...
public class Main {
    ...
    private CompletionStage<Done> run() throws Exception {
        ...
        List<Movie> primaryMovies = Arrays.asList(new Movie(23, "Psycho"), new Movie(423, "Citizen Kane"));
        CompletionStage<Done> writing = helper.writeToKafka(topic, primaryMovies, actorSystem);
        ...
        return reading.thenCompose(
                movies -> {
                    movies.forEach(movie -> System.out.println("read " + movie));
                    try {
